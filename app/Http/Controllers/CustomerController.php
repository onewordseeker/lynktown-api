...
class CustomerController extends Controller
{
    public function feed(Request $request)
    {
        ...
        $lynks->load('products.product');
        $lynks->load('store');
        $lynks->load('store.logo');
        $lynks->load('wishlist');

        // Calculate the total price for each Lynk
        $lynks->each(function ($lynk) {
            $totalPrice = $lynk->products->sum(function ($product) {
                return $product->product->price ?? 0;
            });

            $lynk->total_price = $totalPrice;
        });
